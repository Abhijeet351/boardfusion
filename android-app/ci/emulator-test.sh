...
node webview-test.js offline || rc=1
echo "== network back"; neton
node webview-test.js online-recover || rc=1

echo "== fresh launch + game"; adb shell am force-stop $PKG; sleep 1
adb shell am start -W -n $PKG/.MainActivity
node webview-test.js game || rc=1
...
if [ -n "$pid_before" ] && [ "$pid_before" = "$pid_after" ]; then echo "PASS app kept running after back ($pid_before)"; else echo "FAIL app process changed ($pid_before -> $pid_after)"; rc=1; fi
