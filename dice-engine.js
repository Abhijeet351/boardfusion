...
function roll(g,seat,rng=Math.random){guard(g,seat);if(g.dice!==null||g.awaiting)throw Error('already rolled');const d=1+Math.floor(rng()*6),c=g.players[g.turn].color;g.dice=d;g.sixes=d===6?g.sixes+1:0;if(g.sixes===3){g.sixes=0;advance(g);return event(g,'forfeit',{rolled:d,actor:seat});}const mv=movable(g,c,d);if(!mv.length){advance(g);return event(g,'no-move',{rolled:d,actor:seat});}g.awaiting=true;g.revision++;return event(g,'rolled',{dice:d,movable:mv});}
