...
g=E.create(['A','B']);g.tokens.red[0]=55;ev=E.roll(g,0,die(2));assert.equal(ev.type,'no-move');assert.equal(ev.rolled,2);assert.equal(ev.actor,0);assert.equal(ev.state.dice,null);assert.equal(g.turn,1);assert.equal(g.tokens.red[0],55);
// capture is server-calculated
g=E.create(['A','B']);g.tokens.red[0]=4;g.tokens.yellow[0]=36;E.roll(g,0,die(6));ev=E.move(g,0,0);assert.equal(ev.capture,1);assert.equal(g.tokens.yellow[0],-1);
// triple six forfeits even if client asks to move
g=E.create(['A','B']);E.roll(g,0,die(6));E.move(g,0,0);E.roll(g,0,die(6));E.move(g,0,0);ev=E.roll(g,0,die(6));assert.equal(ev.type,'forfeit');assert.equal(ev.rolled,6);assert.equal(ev.actor,0);assert.equal(ev.state.dice,null);assert.equal(g.turn,1);assert.throws(()=>E.move(g,0,0),/out of turn/);
