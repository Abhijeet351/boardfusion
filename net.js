...
window.NET=(function(){
 ...
 function open(target){url=target;closing=false;return new Promise((resolve,reject)=>{const current=new WebSocket(url);ws=current;current.onopen=()=>{retries=0;resolve();};current.onerror=reject;current.onmessage=e=>{let m;try{m=JSON.parse(e.data)}catch(_){return;}if(m.t==='v2-welcome'){room=m.code;seat=m.seat;localStorage.setItem(key(room),JSON.stringify({code:room,token:m.token,name,url}));}if(m.t==='v2-resumed'){room=m.code;seat=m.seat;emit('resumed',m);}if(m.t==='v2-displaced'){closing=true;emit('error',{reason:'This seat was resumed in another tab or device. Reload there to keep playing.'});return;}if(m.t==='v2-muted')muted=!!m.value;emit(m.t,m);};current.onclose=e=>{if(ws!==current)return;if(e&&e.code===4001){closing=true;emit('error',{reason:'This seat was resumed in another tab or device. Reload there to keep playing.'});return;}emit('closed',{});if(!closing&&room)setTimeout(reconnect,Math.min(8000,500*Math.pow(2,retries++)));};});}
